...
# On Windows the tools pass backslash paths, which no directory pattern below would match, and the
# filesystem is case-insensitive, so NODE_MODULES\x must match too.
