...
- The root sheet uses `velvet74-half.kicad_sch` twice, as sheets Left and Right. Left keeps the references; Right adds 200 (C references reach 165, so +100 would collide). Edit the half and both follow. Never run Annotate with reset: it renumbers both halves.
...
- **"Update PCB from Schematic" strips `(jumper_pad_groups ...)` from the board footprints**, although the library footprint still declares them. On U2/U4/U202/U204 those groups are what make D+ flow through the USBLC6 package (pads 1 and 6 are one node inside the die, 3 and 4 likewise), so losing them turns 0 unconnected items into 6. It reports nothing: the only warning it prints is the unrelated "no pin 9 in symbol" for the flash thermal pads. After any schematic update check `grep -c jumper_pad_groups velvet74.kicad_pcb` (expect 4) and the DRC unconnected count (expect 0).
- U3/U203 pad 9 is the WSON exposed pad and the flash symbol has only 8 pins, so it carries no net. That is deliberate: the pad has copper and a mask opening but no paste, so it is not soldered, and the ZD25Q128C datasheet asks for nothing. The "no net found for pad 9" warning on every schematic update is expected.
- **A `pcbnew.LoadBoard` then `Save` round trip re-anchors dangling track ends**, so a script that only meant to change a field can edit hand routing. It moved a stray `/Left/SD1` end 0.275 mm once. Prefer a text edit for field-only changes, and diff pad nets and track geometry against HEAD afterwards either way.
